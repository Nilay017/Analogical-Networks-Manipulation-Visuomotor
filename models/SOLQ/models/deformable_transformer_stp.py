    ...
    def forward(
        ...
        if self.two_stage:
            ...
            if args.use_action_ghost_nodes:
                ...
                # prepare action prediction query
                query_embed, tgt = torch.split(query_embed, c, dim=1)
                query_embed = query_embed.unsqueeze(0).expand(bs, -1, -1)
                tgt = tgt.unsqueeze(0).expand(bs, -1, -1)
            if args.init_obj_lang_query:
                # init language embedding as an additional query
                query_embed = torch.cat([self.lang_query_embed.weight.unsqueeze(0).expand(bs, -1, -1), query_embed], dim=1)
                the_id = 518 if args.use_clip_text_encoder else 5 # 5 is id of "the", object id is always after "the" in our case
                object_lang_inds = torch.where(tokenized['input_ids']==the_id) 
                # self.tokenizer.convert_ids_to_tokens(518)
                obj_tokens = text_memory[object_lang_inds[0],object_lang_inds[1]+1].unsqueeze(1)
                tgt = torch.cat([obj_tokens, tgt], dim=1)
            ...
            if args.do_decoder_2d3d:
                ...
                if args.do_deformable_atn_decoder:
                    reference_points = torch.cat([reference_points, self.reference_points(query_embed).sigmoid()], dim=1)
                # concat queries 
                query_embed = torch.cat([query_embed_, query_embed], dim=1)
                tgt = torch.cat([tgt_, tgt], dim=1)
            init_reference_out = reference_points
        else:
            # prepare object + action prediction queries
            query_embed, tgt = torch.split(query_embed, c, dim=1)
            query_embed = query_embed.unsqueeze(0).expand(bs, -1, -1)
            tgt = tgt.unsqueeze(0).expand(bs, -1, -1)
            if args.do_deformable_atn_decoder:
        ...
        if mem_srcs is not None:
            # demo/memory tokens
            src_flatten_mem = mem_srcs.flatten(1,2) # B x seq x hidden_size
            mask_flatten_mem = mem_masks.flatten(1,2) # B x seq
            lvl_pos_embed_flatten_mem = mem_pos_embeds.flatten(1,2) # B x seq x hidden_size

            # st()
            if self.single_atn_history_and_memory:
                # add indicator of memory or history
                lvl_pos_embed_flatten_obj += self.history_mem_embed[0].view(1, 1, self.d_model).expand(B, lvl_pos_embed_flatten_obj.shape[1], self.d_model)
                lvl_pos_embed_flatten_mem += self.history_mem_embed[1].view(1, 1, self.d_model).expand(B, lvl_pos_embed_flatten_mem.shape[1], self.d_model)
                src_flatten_obj = torch.cat([src_flatten_obj, src_flatten_mem], dim=1)
                mask_flatten_obj = torch.cat([mask_flatten_obj, mask_flatten_mem], dim=1)
                lvl_pos_embed_flatten_obj = torch.cat([lvl_pos_embed_flatten_obj, lvl_pos_embed_flatten_mem], dim=1)
                dec_mem_dict = None
            else:
                dec_mem_dict = {"feat":src_flatten_mem, "pos_enc":lvl_pos_embed_flatten_mem, "mask":mask_flatten_mem} # text
        else:
            dec_mem_dict = None
    ...
    def forward(self, 
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if args.do_decoder_2d3d:
                ...
                else: # for pretraining only dont do second pass since not doing loss
                    output_a = dec_query_dict["feat_action"]
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        if (args.within_demo or args.retrieve_knn_demos_subgoal) and not args.single_atn_history_and_memory:
            # Cross attention queries -> memory demo
            self.multihead_attn_m = nn.MultiheadAttention(
                d_model, nhead, dropout=dropout
            )
    ...
    def forward(
        ...
        if (args.within_demo or args.retrieve_knn_demos_subgoal) and dec_mem_dict is not None and not args.single_atn_history_and_memory:
            # Cross attention queries -> memory demos
            tgt2 = self.multihead_attn_m(query=self.with_pos_embed(tgt, dec_query_dict[pos_enc_label]).transpose(0, 1),
                                    key=self.with_pos_embed(dec_mem_dict["feat"], dec_mem_dict["pos_enc"]).transpose(0, 1),
                                    value=dec_mem_dict["feat"].transpose(0, 1), attn_mask=None,
                                    key_padding_mask=dec_mem_dict["mask"])[0]
            tgt = tgt + self.dropout_m(tgt2.transpose(0, 1))
            tgt = self.norm_m(tgt)
...
# NOTE: Deformable + not deformable is handled in TransformerDecoder
# ############# DEFORMABLE DECODER ###############
